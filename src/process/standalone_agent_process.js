...
export class StandaloneAgentProcess {
    start(profile, load_memory = false, init_message = null, count_id = 0, task_path = null, task_id = null) {
        ...
        let args = ['src/process/init_standalone_agent.js', this.name];
        args.push('-p', profile);
        ...
        agentProcess.on('exit', (code, signal) => {
            console.error(`Standalone agent process exited with code ${code} and signal ${signal}`);
            this.running = false;
            ...
            if (code !== 0 && signal !== 'SIGINT') {
                if (Date.now() - last_restart < 10000) {
                    console.error(`Standalone agent process ${profile} exited too quickly and will not be restarted.`);
                    return;
                }
                console.log('Restarting standalone agent...');
        ...
        agentProcess.on('error', (err) => {
            console.error('Standalone agent process error:', err);
        });
